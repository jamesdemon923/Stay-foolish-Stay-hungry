...
*/
	for (int i = 2; i <= s.size(); i++) 
		//start with i as 2 because 1 or 0 = sorted!
		//keep going until i reaches size: accounted for whole shelf!
	{
		Sensor sortMe = s[i - 1]; //last index object
		int j = i - 2; //j starts at next-to-last index
		while (j >= 0 && comp(sortMe, s[j]))
			//shift values until the proper slot is found
		{
			s[j + 1] = s[j];
			j--;
		}
		s[j + 1] = sortMe; //store sortMe into vacated slot
	}
}
...
int main()
{
	...
	// destruction time.
	{
		...
		for (int addMe = 0; addMe < pointerArray.size(); addMe++)
		{
			sensors[addMe] = *pointerArray[addMe];
		}
